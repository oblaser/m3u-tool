...
// clang-format off
#define PRINT_ERROR(msg)        { if (!quiet) { app::printError((msg)); ++msgCnt; } }
#define PRINT_INFO(msg)         { if (!quiet) { app::printInfo((msg)); ++msgCnt; } }
#define PRINT_WARNING(msg)      { if (!quiet) { app::printWarning((msg)); ++msgCnt; } }

#define PRINT_INFO_V(msg)       { if (verbose) { app::printInfo((msg)); ++msgCnt; } }
#define PRINT_WARNING_V(msg)    { if (verbose) { app::printWarning((msg)); ++msgCnt; } }
// clang-format on
...
public:
    processor_exit()
        : std::runtime_error("processor_exit " + std::to_string(EC_ERROR)), m_ec(EC_ERROR)
    {}
    explicit processor_exit(int ec)
        : std::runtime_error("processor_exit " + std::to_string(ec)), m_ec(ec)
    {}
...
static inline void checkCreateOutDir(app::MessageCounter& msgCnt, const app::Flags& flags, const std::filesystem::path& outDirPath)
{
    app::checkCreateOutDir(msgCnt, flags, outDirPath, outDirPath.u8string());
}

void checkOutFile(app::MessageCounter& msgCnt, const app::Flags& flags, const std::filesystem::path& outFilePath, const std::string& fileDisplayPath,
