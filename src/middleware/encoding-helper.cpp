        ...
        if (str)
        {
            ...
            do
            {
                ...
                if (res <= 0)
                {
                    ...
                    {
                        throw std::invalid_argument(acptows_fnNamePrefix OMWi_DISPSTR("invalid arguments"));
                    }
                    ...
                    {
                        throw std::runtime_error(acptows_fnNamePrefix OMWi_DISPSTR("internal error (invalid flags)"));
                    }
                    else if (res < 0)
                    {
                        throw std::runtime_error(acptows_fnNamePrefix OMWi_DISPSTR("Windows API error, MultiByteToWideChar() ") + std::to_string(res) + OMWi_DISPSTR(", GetLastError() ") + std::to_string(err));
                    }
                    else
                    {
                        throw std::runtime_error(acptows_fnNamePrefix OMWi_DISPSTR("internal error"));
                    }
                }
            }
            while (res <= 0);
...
std::string enc::acptou8(const std::string& str)
{
    return omw::windows::wstou8(omw_::acptows(str));
}
